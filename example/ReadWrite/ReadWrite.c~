...
int main()
{
	...
	while(1)
	{
		...
		if(getchar() == 0x1b)
			break;
		printf( "Input and angle between 0 and 300 degrees\n");
		// goalinput = getchar();
		scanf( "%d", &goalinput);
		goalpos = (double)goalinput*1024/300;
		//(int) goalpos;
		printf( "Goal Input: %d   Goal Position: %d\n", goalinput, goalpos );

		// Write goal position
		// dxl_write_word( DEFAULT_ID, P_GOAL_POSITION_L, GoalPos[index] );
		dxl_write_word( DEFAULT_ID, P_GOAL_POSITION_L, goalpos );
		do
		{
